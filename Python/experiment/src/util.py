...
logging.basicConfig(filename=os.path.join(os.getcwd(),'../log/util_log_%s.txt' % const.DATASET_NAME),level=logging.DEBUG,format='%(asctime)s-%(levelname)s:%(message)s')
...
def getMAEandRMSE(recDict,playlistDict,songDict,topN = const.TOP_N):
  ...
  for pid in playlistDict.keys():
    ...
    for i in range(0,recNum):
      ...
      #logging.info('recSid = %s' % str(recTopicDict))
      #logging.info('lastSid = %s' % str(tarDict))
      #logging.info('error between %d and %d = %f' % (recSid,lastSid,recError))
      totalError += recError
    #logging.info('recNum = %d' % recNum)
    if recNum == 0:
      recNum = 0.0001
