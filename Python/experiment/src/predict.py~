...
#get predicted topic dict of next song by average with most simlilar
def topicDictForNextSongByAverageWithSimilar(playlist,songDict,lamda):
  trainingList = playlist.getTrainingList()
  pid = playlist.getPid()
  count = len(trainingList)
  sid = trainingList[count-1]
  ...
  topicDict = {}
  for topic in avgTopicDict.keys():
    pro = lamda*avgTopicDict[topic] + (1 - lamda)*lastTopicDict[topic]
    topicDict[topic] = pro
  return topicDict
