...
public class SongTopicReader {
	...
	public static void main(String[] args) throws FileNotFoundException{
		...
		PrintWriter songPw = new PrintWriter(new File("allsong.txt"));
		Iterator<Entry<Integer, HashMap<Integer, Double>>> songIter = songMap.entrySet().iterator();
		while(songIter.hasNext()){
			Entry<Integer, HashMap<Integer, Double>> sMap = songIter.next();
			Integer sid = sMap.getKey();
			songPw.println(sid);
		}
		songPw.close();
		PrintWriter topicPw = new PrintWriter(new File("song_of_topic.txt"));
		Iterator<Entry<Integer, HashMap<Integer, Double>>> topicIter = topicStatsMap.entrySet().iterator();
		while(topicIter.hasNext()){
			Entry<Integer, HashMap<Integer, Double>> tMap = topicIter.next();
			Integer tid = tMap.getKey();
			HashMap<Integer,Double> map = tMap.getValue();
			for(Integer sid: map.keySet()){
				topicPw.println(tid+"\t"+sid);
			}
		}
		topicPw.close();
	}
